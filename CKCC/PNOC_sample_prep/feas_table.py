...
def decrypt(infile, outdir, pwfile):
    ...
    #print " ".join( str(i) for i in ssl_command)
...
def concat_files(indir, inputfiles, outputname):
    ...
    for f in inputfiles:
        ...
        #print " ".join( str(i) for i in zcat_command),  '>>', outputname
        with open(outputname, 'a') as f_out:
            subprocess.check_call(zcat_command, stdout=f_out)
    #print "wc -l {}".format(outputname)
    lineCount = 0
    lineCount = int(subprocess.check_output(['wc', '-l', outputname]).split(' ')[0])

    gzip_command = [ 'gzip', 
                     '{}'.format(outputname)]
    #print " ".join( str(i) for i in gzip_command)
    subprocess.check_call(gzip_command)
    return lineCount, outputname + '.gz'
...
def encrypted_upload(myfile, tempkey):
    ...
    #print " ".join( str(i) for i in s3am_command)
...
for i in pnoc:
    ...
    if i.study != curStudy:
        if len(didDecrypt) > 0:
            ...
            #print " ".join( str(i) for i in rm_command)
        ...
        sync_command = ['aws',
                    's3',
                    'sync',
                    '{}'.format(os.path.join(awsbasedir, i.basedir)+'/'), 
                    '{}'.format(outdir)]
    ...
    decryptdir = os.path.join(outdir, i.uuid)
    #DEBUG os.mkdir(decryptdir)
    #print "mkdir", decryptdir
    for f in i.fastq:
        print "HI THERE", f
    ...
    # concatenate
    r1count, r1zip = concat_files(decryptdir, i.r1, '{d}/{u}_{s}.R1.fastq'.format(d=os.path.join(decryptdir), u=i.uuid, s=i.stype ))
    r2count, r2zip = concat_files(decryptdir, i.r2, '{d}/{u}_{s}.R2.fastq'.format(d=os.path.join(decryptdir), u=i.uuid, s=i.stype ))
    if r1count != r2count:
        print >>sys.stderr, "ERROR, concatenated fastq files have different linecounts"
        sys.exit(1)
    ...
    print '{},{},{}'.format(i.uuid, r1url, r2url)
